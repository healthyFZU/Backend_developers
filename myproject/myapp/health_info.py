...
# 创建用户健康信息 API
@health_info_blueprint.route('/user/<string:userId>/health-info', methods=['POST'])
def create_user_health_info(userId):
    ...
    # 插入新的健康信息记录
    cur.execute("INSERT INTO user_health_info (userId, blood_sugar, blood_pressure, allergens, recent_medications, past_medical_history, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                (userId, blood_sugar, blood_pressure, allergens, recent_medications, past_medical_history, created_at, updated_at))
...
# 获取用户健康信息 API
@health_info_blueprint.route('/user/<string:userId>/health-infos', methods=['GET'])
def get_user_health_info(userId):
    ...
    if health_info:
        return jsonify({
            ...
            'created_at': health_info[6],
            'updated_at': health_info[7]
        }), 200
    else:
        return jsonify({'message': '用户健康信息不存在'}), 404

# 更新用户健康信息 API
@health_info_blueprint.route('/user/<string:userId>/health-info', methods=['PUT'])
def update_user_health_info(userId):
    ...
    if cur.rowcount == 0:
        return jsonify({'message': 'Health info not found or not updated!'}), 404
    else:
        return jsonify({'message': 'Health info updated successfully!'}), 200

# 删除用户健康信息 API
@health_info_blueprint.route('/user/<string:userId>/health-info', methods=['DELETE'])
