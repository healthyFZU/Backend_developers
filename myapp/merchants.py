...
def get_merchants():
    ...
    for merchant in merchants:
        merchants_list.append({
            ...
            "address": merchant[2],
            "logo": merchant[3],
            "menu": json.loads(merchant[6]) if merchant[6] else None,  # 解析菜单为JSON对象
            "created_at": merchant[4],
            "updated_at": merchant[5]
        })
...
def get_merchant(merchantId):
    ...
    if merchant_info:
        return jsonify({
            ...
            "menu": json.loads(merchant_info[6]) if merchant_info[6] else None,  # 解析菜单为JSON对象
